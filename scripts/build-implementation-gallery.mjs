...
<main>
  <header>
    ...
    <div class="badges">
      ...
      <span class="badge"><strong>18</strong> live doctor checks</span>
  ...
  <section id="python-regression">
    <h2>The “enabled but no Python” regression</h2>
    <div class="grid">
      <article class="card"><div class="icon">×</div><h3>Observed state</h3><p>Both plugins were loaded, but <code>getTypeByExtension("py")</code> returned undefined. Direct vault Python files had no view owner.</p></article>
      <article class="card"><div class="icon">↯</div><h3>Root cause</h3><p>The configured list included <code>svg</code>, already owned by Obsidian's image view. VSCode Editor 1.0.5 registers the whole list atomically, so one collision aborted every extension.</p></article>
      <article class="card"><div class="icon">✓</div><h3>Verified repair</h3><p><code>svg</code> stays with <code>image</code>; <code>py</code> now maps to <code>vscode-editor</code>. Direct and bridge-backed Python files both opened in Monaco in isolated Obsidian.</p></article>
    </div>
  </section>
  ...
  <section>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>Direct vault Python</td><td><code>py → vscode-editor</code>; ordinary <code>direct.py</code> opened in Monaco</td><td>Pass</td></tr>
        ...
        <tr><td>Environment diagnosis</td><td><code>link-doctor</code> asserts runtime <code>.py</code> ownership, not just plugin enablement</td><td>18 / 18</td></tr>
      </tbody>
    </table>
  </section>

  <footer>Forked from Folder Bridge 2.15.3 · local-only runtime · screenshots captured on invisible throwaway vaults · Python ownership regression verified 2026-09-01</footer>
</main>
<script>
const proofs = {
